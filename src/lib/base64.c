...
struct base64_decoding_buffer {
	unsigned char *s;
	size_t len;
	unsigned char *encoded; //Data which has not been encoded so far (input length was not dividable by 4 or not padded)
	size_t encoded_len;
...
void base64_encode_chunk(struct base64_encoding_buffer *buf, unsigned char *s, size_t len) {
	...
	if( buf->s != NULL ) {
        ...
        unsigned char *p = buf->s + buf->encoded_len;
        while( *p == '\r' || *p == '\n' || *p == '\0')
            p--;

		if( *p == '=' ) {
			fprintf(stderr, "Cannot append chunk, buffer was padded (please take care to choose a length dividable by 3 when appending chunks; only last chunk can have different size)\n");
			exit(EX_IOERR);
		}
	}
